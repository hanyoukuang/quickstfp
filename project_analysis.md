...
> 生成时间: 2026-06-02 | 分析维度: 架构分层、并发模型、数据流、可测试性、资源管理
...
│  main.py           (编排入口, 窗口管理)            │
├─────────────────────────────────────────────────┤
│  ui/views/         (视图层, QWidget 子类, 11文件)   │
│  ui/components/    (可复用组件, 2文件)              │
├─────────────────────────────────────────────────┤
│  core/session.py   (QThread + asyncio 事件循环)    │
│  core/transport.py (传输引擎, Signal 解耦)          │
├─────────────────────────────────────────────────┤
│  database/         (SQLite DAO + Fernet 加密)     │
│  utils/            (文件工具, 二进制检测)           │
└─────────────────────────────────────────────────┘
```

**依赖方向**: `utils → core → database → ui → main.py`。**无循环依赖** ✅ — 这是架构最干净的部分。

**跨层通信机制**: core → ui 通过 **Qt Signal 进行彻底解耦**。`Transport` 类从不导入任何 UI Widget，仅通过 `Signal(int)` / `Signal(str)` 等原始类型信号向上层广播状态。这是教科书级的 **Observer 模式** 实现。

---

## 🔴 架构层面严重问题

### ARCH-1: SSHSFTPInfo 是 God Object — `core/session.py`

**26 个方法，218 行**，承担了三种完全不相关的职责：

| 职责域 | 方法 |
|--------|------|
| 线程生命周期 | `run()`, `close_session()`, `quit()` (QThread) |
| SSH 连接管理 | `get_session()`, `wait_for_connection()` |
| 文件系统操作 | `is_file()`, `chdir()`, `getcwd()`, `read_file()`, `save_file()`, `del_file()`, `makedirs()`, `copy_file()`, `move_file()`, `rename()`, `get_file_size()`, `get_permissions()`, `chmod()`, `realpath()` |

**问题**: 任何需要远程文件操作的代码都必须持有整个 `SSHSFTPInfo` 对象（包括线程、事件循环、凭据），违反了 **接口隔离原则 (ISP)**。

**建议拆分**:
```python
class SSHConnection(QThread):       # 只管连接与线程
    async def connect(...) -> None
    def wait_for_connection() -> None
    def close_session() -> None

class SFTPFileManager:              # 只管文件操作
...
file_mgr = SFTPFileManager(connection)
```

**实现估算**: ~100 行重构 (纯内部重构, 不改变外部接口)。

---

### ARCH-2: RemoteFileWidget 职责过载 — `ui/views/remote_file_widget.py`

**22 个方法, 302 行**，同时承担：

- 目录树渲染 (继承自 `BaseRemoteTreeWidget`)
- 右键菜单管理 (`show_context_menu()`, 45行)
- 文件 CRUD (`del_item`, `rename`, `makedir`, `new_file`)
- 剪贴板系统 (`move_paths`, `copy_paths`, `paste_items`, `put_items`)
- 外部编辑器集成 (`open_external`, `ExternalEditorWatcher`)
- 权限管理 (`change_permissions`, `PermissionDialog`)
- 下载触发 (`download_item`)

**建议**: 将剪贴板系统抽离为独立 `ClipboardManager` 混入类，右键菜单拆分为 `ContextMenuBuilder`。
...
| 线程 | 用途 | 事件循环 |
|------|------|----------|
| 主线程 (UI) | PySide6 事件循环 | Qt event loop |
| 后台线程 | `SSHSFTPInfo` QThread | asyncio event loop |

跨线程通信使用:
- `asyncio.run_coroutine_threadsafe()` — UI → 后台 (11处)
- Qt Signals — 后台 → UI (所有进度、文件列表、终端输出)

**设计评价**: 双线程 + Signal解耦 是 PySide6 + asyncssh 的正确方案 ✅。但具体实现中存在以下问题：

---

### CONC-1: `PUT.search_transport_file()` 同步阻塞 asyncio 事件循环 — `core/transport.py:427-438`

```python
class PUT(Transport):
    def search_transport_file(self, src: str, loc: str) -> int:  # ⚠️ 同步方法!
        total_size = 0
        for entry in os.scandir(src):           # os.scandir() 是同步阻塞调用
            if entry.is_dir():
...
然后从异步上下文中调用:
```python
async def transport(self) -> None:              # transport.py:440
    all_size = self.search_transport_file(src, loc)  # ⚠️ 阻塞 asyncio 循环!
```

**对比 `GET` 类的实现 (正确)**:
```python
class GET(Transport):
    async def search_transport_file(self, src, loc):  # ✅ 异步方法
        async for entry in self.sftp.scandir(src):     # asyncssh 的异步 scandir
```

**风险**: 扫描大型本地目录时阻塞 asyncio 事件循环，期间所有 SSH 终端 I/O、其他并发传输任务全部暂停。

**建议**: 将 `PUT.search_transport_file` 改为异步方法:
```python
async def search_transport_file(self, src, loc):
    # 使用 run_in_executor 将同步 os.scandir 卸载到线程池
    loop = asyncio.get_event_loop()
    # ... 或直接将递归逻辑改为 async，在叶子节点使用 await loop.run_in_executor(...)
```

---

### CONC-2: 传输进度追踪缺少同步保护 — `core/transport.py:149-176`

```python
class ProgressTracker:
    def __call__(self, _src, _loc, now_size, _all_size):
        delta = now_size - self.last_size
        self.transport._total_progress_size += delta      # ⚠️ 并发写入
        self.transport.progress_updated.emit(...)

    def handle_fail(self, all_size, src):
        self.transport._total_progress_size += delta      # ⚠️ 并发写入
        self.transport.transport_fail_filename += f"{src}\n"  # ⚠️ 并发字符串拼接
```

多个 `ProgressTracker` 实例（每个并发文件传输一个）在同一事件循环中并发更新共享的 `_total_progress_size` 和 `transport_fail_filename`。虽然 Python GIL 防止了字节码级别的崩溃，但在 `_on_transport_done()` (line 238-251) 读取这些值时可能看到不一致的状态。

**建议**: 为共享状态添加 `asyncio.Lock`:
```python
...
### CONC-3: 嵌套 QEventLoop 反模式 — `core/session.py:50-82`

```python
def wait_for_connection(self):
    event_loop = QEventLoop()      # ⚠️ 嵌套事件循环
    timer = QTimer()
    timer.timeout.connect(lambda: event_loop.quit() if self.connect_event.is_set() else None)
    timer.start(50)
    event_loop.exec()              # ⚠️ 阻塞但保持 UI 刷新

def _wait_future(self, future):
    event_loop = QEventLoop()      # ⚠️ 又一个嵌套事件循环
    timer = QTimer()
    timer.timeout.connect(lambda: event_loop.quit() if future.done() else None)
    timer.start(20)
    event_loop.exec()
...
**问题**:
1. **无超时机制** — 如果 future 永远不完成 (如 DNS 解析挂起)，嵌套循环永远不退出
2. **可重入风险** — 嵌套事件循环期间，用户点击按钮的处理函数会在内层循环中执行，可能导致意外状态
3. **调用链污染** — `_run_sync()` → `_wait_future()` 传播到所有同步文件操作，形成深层嵌套

**建议**: 重新设计为纯异步 + Signal 模式:
```python
class SSHSFTPInfo(QThread):
    ...
    def run(self):
        ...
        self.connection_ready.emit()  # 通过 Signal 通知主线程
        self.loop.run_forever()
```

---

### CONC-4: TerminalBridge 无限循环无取消机制 — `ui/components/terminal_widget.py:32-43`
...
虽然 `close_session()` 通过 `task.cancel()` 可以终止此协程，但如果在 `await read()` 期间连接异常，会静默退出。建议添加 `asyncio.CancelledError` 显式处理和连接状态检查。
...
项目存在多处**隐式全局状态**，均通过 CWD 相对路径访问共享文件：

| 文件 | 路径 | 风险 |
|------|------|------|
| `database/user_model.py:50` | `userinfo.db` | 从不同目录运行 → 不同数据库 |
| `database/user_model.py:15` | `.secret.key` | 从不同目录运行 → 不同密钥 (旧凭据失效) |
| `ui/views/snippets_widget.py:56` | `quick_snippets_v2.json` | 多会话可能覆盖相同文件 |
| `ui/views/remote_file_widget.py:255` | `./tmp/` | 硬编码相对路径, 无法清理 |

**建议**: 引入统一的配置目录 (如 `~/.config/quickstfp/`):
```python
...
class UserInfoDB:
    def __init__(self, db_path: str = None):
        self.db_path = db_path or str(CONFIG_DIR / "userinfo.db")
```
...
```
SiteManagerWidget  ──直接 new──→ UserInfoDB()          ❌ 无法 Mock
SiteManagerWidget  ──直接 new──→ CryptoManager()       ❌ 无法 Mock (通过 UserInfoDB)
SFTPTabWidget      ──直接 new──→ SSHSFTPInfo()         ❌ 无法 Mock (需要真实SSH)
SFTPTabWidget      ──直接 start→ info.start()           ❌ 启动即发网络连接
QuickSnippetsWidget ──直接读写→ quick_snippets_v2.json  ❌ 文件系统耦合
ExternalEditorWatcher ──直接 new→ tempfile.mkdtemp()    ❌ 文件系统耦合
```

**结果**: 零测试覆盖率（无 `pytest`、无 `conftest.py`、无 Mock）并非巧合 — 现有架构使单元测试极为困难。

**建议**: 首先为核心层引入接口抽象:
```python
from abc import ABC, abstractmethod

class ISFTPConnection(ABC):
    @abstractmethod
...
class SSHSFTPInfo(QThread, ISFTPConnection):  # 实现接口
    ...

# 测试时注入 Mock
def test_remote_file_widget():
    mock_conn = MockSFTPConnection()
    widget = RemoteFileWidget(connection=mock_conn)  # 可测试!
```
...
### DUP-1: `refresh()` 和 `search()` 被子类重复实现

`RemoteFileWidget` 继承 `BaseRemoteTreeWidget`，但**完全重写**了以下方法且内容完全一致：

| 方法 | 基类 (`base_remote_tree.py`) | 子类 (`remote_file_widget.py`) |
|------|------|------|
| `refresh()` | L108-113 | L123-129 (完全相同) |
| `search()` | L115-120 | L131-137 (完全相同) |

应从子类中删除这些重写方法，直接使用继承。

### DUP-2: `MAX_PREVIEW_SIZE` 定义两次

`remote_file_widget.py:101` 和 `:270` — 同一个常量在 `open_external()` 和 `double_item()` 中各定义一次，应提升为类常量。

### DUP-3: GET/PUT 传输方法高度相似

- `GET._transport_file` (L291-336) vs `PUT._transport_file` (L379-425) — **~75% 相同逻辑**
- `GET.transport()` (L358-369) vs `PUT.transport()` (L440-453) — **~90% 相同逻辑**

建议提取公共基类方法 `_transport_chunked()`。

---

## 🟢 低层逻辑缺陷

### LOGIC-1: 端口字段缺少输入验证 — `site_manager.py:301,358`

```python
port = int(self.port_edit.text().strip() or 22)  # ⚠️ ValueError 崩溃
```

如果用户输入非数字字符 (`"22abc"`, `""` 等)，`int()` 抛出 `ValueError` 且未捕获。
...
### LOGIC-2: `selectedItems()` 方法遮蔽 Qt 原生方法 — `base_remote_tree.py:102-104`

```python
class BaseRemoteTreeWidget(QTreeView):
    def selectedItems(self):          # ⚠️ 遮蔽 QAbstractItemView.selectedItems()
        indexes = self.selectionModel().selectedIndexes()
        return [self.model.itemFromIndex(idx) for idx in indexes if idx.column() == 0]
```

Qt 原生的 `selectedItems()` 返回 `QModelIndex` 列表，而此重写返回 `QStandardItem` 列表。虽然方便，但类型不一致会导致 IDE 和类型检查器混淆。建议改名为 `get_selected_items()` 或在文档中明确标注。

---

### LOGIC-3: 类级可变默认值反模式 — `core/session.py:23`

```python
class SSHSFTPInfo(QThread):
    connect_is_ready: bool = False  # ⚠️ 类属性, 非实例属性
```

虽然 `__init__` 中通过 `self.connect_is_ready = False` 正确创建了实例属性进行遮蔽，但类级声明是死代码，容易误导维护者。

同样的问题存在于 `TransportTargetWidget.abspath: str = ""` — `transport_widgets.py:19`。

---

### LOGIC-4: Mixin 隐式依赖 — `remote_drag_drop.py`

```python
class RemoteDragDropMixin:
    def startDrag(self, supportedActions):
        items = self.selectedItems()             # 依赖 BaseRemoteTreeWidget 提供
        paths = [self.get_item_path(item) ...]   # 依赖 BaseRemoteTreeWidget 提供
```

`RemoteDragDropMixin` 没有显式声明它依赖的方法 (`selectedItems()`, `get_item_path()`, `model` 等)，这些方法由 `BaseRemoteTreeWidget` 提供，但依赖关系仅在 MRO 中隐式存在：

```python
class RemoteFileWidget(RemoteDragDropMixin, BaseRemoteTreeWidget):  # 脆弱的 MRO
```

**建议**: 使用 Protocol 显式声明依赖:
```python
from typing import Protocol

class HasRemoteFileOps(Protocol):
    def selectedItems(self) -> list[QStandardItem]: ...
    def get_item_path(self, item: QStandardItem) -> str: ...

class RemoteDragDropMixin:
    def startDrag(self: HasRemoteFileOps, supportedActions): ...
```
...
### RES-1: 图标临时目录永不清理 — `base_remote_tree.py:53`

```python
self.icon_temp_dir = tempfile.mkdtemp(prefix="quickstfp_icons_")  # 创建但从不删除
```

`get_file_icon()` 在此目录中创建 0 字节占位文件以获取系统图标，该目录在 `BaseRemoteTreeWidget` 关闭时从不清理。相比之下，`ExternalEditorWatcher` 的 `cleanup_temp_files()` 正确清理了自己的临时目录。

**建议**: 在 `closeEvent` 或析构函数中添加 `shutil.rmtree(self.icon_temp_dir)`。

### RES-2: 数据库连接未显式关闭 — `database/user_model.py`

```python
class UserInfoDB:
    def __init__(self, db_path='userinfo.db'):
        self.conn = sqlite3.connect(self.db_path)  # 打开但从不关闭

    def close(self):             # ✅ 方法存在
        if self.conn:
            self.conn.close()

    def __enter__(self): ...     # ✅ 上下文管理器实现存在
    def __exit__(self): ...
```

然而 `SiteManagerWidget` 创建 `UserInfoDB()` 时不使用 `with` 语句 (line 24)，也不在 `closeEvent` 中调用 `.close()`。连接依赖 GC 回收。

**建议**: 在 `SiteManagerWidget.closeEvent` 中调用 `self.userinfo_db.close()`。
...
### 第一阶段: 消除 God Object (架构健壮性)

1. **ARCH-1**: 拆分 `SSHSFTPInfo` 为 `SSHConnection` + `SFTPFileManager` (~100 行重构)
2. **ARCH-2**: 拆分 `RemoteFileWidget` 右键菜单和剪贴板逻辑 (~80 行)
3. 为核心接口 (`ISFTPConnection`, `ICryptoProvider`) 创建抽象层 (~50 行)

### 第二阶段: 并发安全加固

4. **CONC-1**: `PUT.search_transport_file` 改为异步 (~30 行)
5. **CONC-2**: 为传输共享状态添加 `asyncio.Lock` (~20 行)
6. **CONC-3**: 重构 `wait_for_connection` 为 Signal 驱动 (~60 行)
7. 为 `_wait_future` 添加超时机制 (~15 行)

### 第三阶段: 消除技术债务

8. **DUP-1**: 删除 `RemoteFileWidget` 重复的 `refresh()`/`search()` (~5 行)
9. **DUP-2**: 提升 `MAX_PREVIEW_SIZE` 为类常量 (~3 行)
10. **DUP-3**: 提取 GET/PUT 公共传输逻辑 (~40 行)
11. **LOGIC-1**: 端口输入验证 (~5 行)
12. **RES-1**: 图标临时目录清理 (~5 行)
13. **RES-2**: 数据库连接显式关闭 (~3 行)

### 第四阶段: 可测试性基础

14. 引入 `pytest` + `pytest-asyncio` + `pytest-qt` (配置 ~20 行)
15. 为核心模块编写接口协议 (Protocols) (~40 行)
16. 迁移硬编码路径到 `~/.config/quickstfp/` (~30 行)

> 总预估：新增 ~500 行代码 + ~100 行测试骨架即可完成架构层改进。所有改进均为内部重构，不改变外部行为。
